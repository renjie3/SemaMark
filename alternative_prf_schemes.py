...
def sem_prf(hidden_embeddings: torch.LongTensor, salt_key:int, cl_mlp, discrete_value_number) -> int:
    # print(hidden_embeddings)
    # print(torch.norm(hidden_embeddings, p=2, dim=1))
    # print(hidden_embeddings.shape)
    # import pdb; pdb.set_trace()
    # input("hidden_embeddings")
    ...
    # torch.save(hidden_embeddings[-1:, :], "./results/test_hidden_emb.pt")
    # input("check saved")
    # pos_1 = torch.load("./results/test_pos_1.pt")
    # out_1 = cl_mlp(pos_1)

    # theta = torch.atan2(out_1[:, 1], out_1[:, 0])

    # for i in range(len(theta)):
    #     if theta[i] < 0:
    #         theta[i] += 2*3.1415

    #     print(theta[i] / (2*3.1415) * 10)

    # input("check")

    # print(xy)
    
    # # shape is different xy = cl_mlp(hidden_embeddings[-1:, :])
    # if xy[0,1] == 0:
    #     if xy[0,0] > 0:
    #         theta = 0
    #     else:
    ...
    # print(theta, theta / (2*np.pi), theta / (2*np.pi) * 10, round(theta / (2*np.pi) * 10))
    a = torch.tensor([round(theta / (2*np.pi) * discrete_value_number + 0.5)])
    # print("check sem_prf")
    # import pdb; pdb.set_trace()
