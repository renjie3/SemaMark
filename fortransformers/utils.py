...
class SampleDecoderOnlyOutput(ModelOutput):
    ...
    attentions: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    wm_green: Optional[Tuple[torch.Tensor]] = None
    wm_seed: Optional[Tuple[torch.Tensor]] = None
...
class GenerationMixin:
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # outputs.hidden_states[24] is the embeddings
            ...
            next_tokens_scores = logits_processor(input_ids, next_token_logits, hidden_embeddings=outputs.hidden_states[24])
            # input("logits type")
            # import pdb; pdb.set_trace()
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        unfinished_sequences = torch.ones(input_ids.shape[0], dtype=torch.long, device=input_ids.device)
        this_peer_finished = False  # used by synced_gpus only
        # next_tokens[1] in new_token_green_list[1]
        ...
        while True:
            ...
            if len(logits_processor) > 0:
                # import pdb; pdb.set_trace()
                if logits_processor[0].mean_pooling:
                    cache_last_hidden_emb.append(outputs.hidden_states[-1])
                    all_last_hidden_emb = torch.cat(cache_last_hidden_emb, dim=1)
                    # print(all_last_hidden_emb.shape)
                    # import pdb; pdb.set_trace()
                    # this is for mean pooling
                    if logits_processor[0].mean_pooling:
                        if logits_processor[0].cl_pooling_method == 'mean':
                            mean_last_hidden_emb = torch.mean(all_last_hidden_emb, dim=1, keepdim=True)
                        elif logits_processor[0].cl_pooling_method == 'former_k':
                            # pdb.set_trace()
                            # import pdb; pdb.set_trace()
                            k = logits_processor[0].k
                        ...
                        elif logits_processor[0].cl_pooling_method == 'weighted_former_k':
                            k = logits_processor[0].k
                            start_id = max(0, all_last_hidden_emb.shape[1] - k)
                            accumulated_emb = 0
                            all_weight_count = 0
                    ...
                    # print(all_last_hidden_emb.shape)
                    # print(mean_last_hidden_emb.shape)
                    # import pdb; pdb.set_trace()
                    next_token_scores, new_token_green_list, seed_list = logits_processor(input_ids, next_token_logits, hidden_embeddings=mean_last_hidden_emb)
                else:
                    next_token_scores, new_token_green_list, seed_list = logits_processor(input_ids, next_token_logits, hidden_embeddings=outputs.hidden_states[-1])
                wm_seed.append(seed_list)
            else:
                next_token_scores = logits_processor(input_ids, next_token_logits, hidden_embeddings=outputs.hidden_states[-1])
            ...
            if len(logits_processor) > 0:
                batch_green_result = []
                for b_idx in range(len(next_tokens)):
                    if next_tokens[b_idx] in new_token_green_list[b_idx]:
                        batch_green_result.append(True)
                    else:
                        batch_green_result.append(False)
                wm_green.append(batch_green_result)
        ...
        if len(wm_green) > 0:
            wm_green = torch.tensor(wm_green).permute((1, 0)).to(input_ids.device)
            wm_seed = torch.tensor(wm_seed).permute((1, 0)).to(input_ids.device)
            # print(wm_green.shape, wm_seed.shape)
            # import pdb; pdb.set_trace()
