...
def load_tokenizer(args):
    ...
    if "llama" in model_name:
        tokenizer = LlamaTokenizer.from_pretrained(model_name, cache_dir=args.hf_cache_dir)
...
def load_detector(args):
    ...
    if "llama" in args.model_name_or_path:
        tokenizer = LlamaTokenizer.from_pretrained(args.model_name_or_path, cache_dir=args.hf_cache_dir)
        tokenizer.pad_token_id = 0  # unk
        model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, cache_dir=args.hf_cache_dir)
    else:
        # print(args.model_name_or_path)
    ...
    if "sem" in args.seeding_scheme:
        watermark_detector = SemWatermarkDetector(
            ...
            cl_mlp=cl_mlp,
            cl_discrete_value_number=args.cl_discrete_value_number,
            cl_mean_pooling=args.cl_mean_pooling,
...
def compute_z_score(
    ...
    if args.prf_type == "sem_prf":
        ...
        prompt_token = example["generation_prompt"]
...
def load_oracle_model(args):
    ...
    if args.load_fp16:
        oracle_model = AutoModelForCausalLM.from_pretrained(
            oracle_model_name, torch_dtype=torch.float16, device_map="auto", cache_dir=args.hf_cache_dir
        )
    else:
        # print("oracle_model_name: ", oracle_model_name)
        # print("args.hf_cache_dir: ", args.hf_cache_dir)
        # import pdb ; pdb.set_trace()
        oracle_model = AutoModelForCausalLM.from_pretrained(oracle_model_name, cache_dir=args.hf_cache_dir)
    if "llama" in oracle_model_name:
        oracle_tokenizer = LlamaTokenizer.from_pretrained(oracle_model_name, cache_dir=args.hf_cache_dir)
        ...
        oracle_tokenizer = AutoTokenizer.from_pretrained(oracle_model_name, cache_dir=args.hf_cache_dir)
    if args.use_gpu:
