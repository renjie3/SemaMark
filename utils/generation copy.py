...
def generate_embedding_pairs_for_rebuttal(
    ...
    # Example tensor
    tensor = torch.rand(wm_tokenized_input.input_ids.shape)  # Replace with your tensor

    # Determine 5% of elements
    num_elements = tensor.size(1)
    num_to_remove = int(num_elements * 0.05)
    ...
    try: 
        ...
        # Compute token embeddings
        with torch.no_grad():
            model_output = sem_model(**wm_tokenized_input)
        ...
        # Compute token embeddings
        with torch.no_grad():
            model_output = sem_model(**nowm_tokenized_input)
